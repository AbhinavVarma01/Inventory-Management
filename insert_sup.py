...
def insert_issuevoucher(depid, itemid, ivnumber, quantity, rvnumber, dateofleaving):
    try:
        cursor.execute("INSERT INTO issuevoucher (depid, itemid, ivnumber, quantity, rvnumber, dateofleaving) VALUES (?, ?, ?, ?, ?, ?)",
                       (depid, itemid, ivnumber, quantity, rvnumber, dateofleaving))
        conn.commit()
...
print("<input type='text' id='empid' name='empid' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'><br>")
...
if "action" in form and form["action"].value == "Insert":
    if all(key in form for key in ['depid', 'itemid', 'ivnumber', 'quantity', 'rvnumber', 'dateofleaving']):
        ...
        insert_employee(depid, itemid, ivnumber, quantity, rvnumber, dateofleaving)
    else:
